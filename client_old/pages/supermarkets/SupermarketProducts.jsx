...
import { SupermarketProductsCollection } from '../../../imports/db/collections';
import { ProductCard } from '../../components/ProductCard';

const pages = [20, 40, 60, 100];

export function SupermarketProducts() {
  ...
  useTracker(() => {
    const handler = Meteor.subscribe('supermarketProducts', id, parseInt(Meteor.user().profile.attributes.postalCode, 10));
    if (!handler.ready()) setProducts([]);
    else {
      const data = SupermarketProductsCollection.find().fetch();
      const components = [];
      data.forEach((element) => {
        components.push(
          {
            component: <ProductCard product={element} />,
            key: element._id,
          },
        );
      });
